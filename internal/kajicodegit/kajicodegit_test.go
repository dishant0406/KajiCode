...
func TestInspectSummarizesChangesAndRedactsDiff(t *testing.T) {
	...
	runner := &fakeRunner{results: []CommandResult{
		...
		{Stdout: "feature/m5\n"},
		{Stdout: "abc1234\n"},
		{Stdout: " M internal/verify/verify.go\x00?? internal/kajicodegit/kajicodegit.go\x00"},
		{Stdout: "abc1234\n"},
		{},
		{},
	...
	if summary.Files[1].Path != "internal/kajicodegit/kajicodegit.go" || summary.Files[1].Status != "untracked" || !summary.Files[1].Untracked {
...
func TestInspectPreviewIncludesUntrackedOnlyChanges(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "notes.md"), "hello kajicode\n")
	...
	if !strings.Contains(summary.Diff, "diff --git a/notes.md b/notes.md") || !strings.Contains(summary.Diff, "+hello kajicode") {
...
func TestInspectBaseRefRealGitDiffsBranchAgainstBase(t *testing.T) {
	...
	runGitCommand(t, root, "-c", "user.name=KajiCode", "-c", "user.email=kajicode@example.invalid", "commit", "-m", "Add feature")
...
func initGitRepo(t *testing.T, withCommit bool) string {
	...
	if withCommit {
		...
		runGitCommand(t, root, "-c", "user.name=KajiCode", "-c", "user.email=kajicode@example.invalid", "commit", "-m", "Initial commit")
	}
	return root
}
...
func TestCreatePRCommandConstruction(t *testing.T) {
	t.Run("CreatePRWithAllOptions", func(t *testing.T) {
		root := t.TempDir()
		runner := &fakeRunner{results: []CommandResult{
			{Stdout: "https://github.com/dishant0406/KajiCode/pull/123\n"},
		}}
		...
		if result.Output != "https://github.com/dishant0406/KajiCode/pull/123\n" {
	...
	t.Run("CreatePRMinimal", func(t *testing.T) {
		root := t.TempDir()
		runner := &fakeRunner{results: []CommandResult{
			{Stdout: "https://github.com/dishant0406/KajiCode/pull/124\n"},
		}}
