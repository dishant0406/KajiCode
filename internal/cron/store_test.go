...
func TestDefaultRootHonorsXDG(t *testing.T) {
	...
	if root != filepath.Join("/tmp/xdg", "kajicode", "cron") {
...
func TestDefaultRootEmptyHomeFallsBackToUserHome(t *testing.T) {
	...
	if filepath.Base(root) != "cron" || filepath.Base(filepath.Dir(root)) != "kajicode" {
		t.Fatalf("DefaultRoot tail = %q, want .../kajicode/cron", root)
	}
}
