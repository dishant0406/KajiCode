...
func verifySetupProvider(deps appDeps, profile config.ProviderProfile) (setupVerification, error) {
	...
	if !profileHasCredential(profile) && !baseURLIsLoopback(profile.BaseURL) {
		...
		return setupVerification{Ran: true, OK: false, Summary: "no api key"},
			fmt.Errorf("no API key found for %s — set its API key (export the provider's API key env var, or pass --api-key-env) or run `kajicode auth`, then re-run setup", name)
	}
...
func setupRequired(resolved config.ResolvedConfig) bool {
	...
	// A stored OAuth login (e.g. `kajicode auth login xai`) is a credential too, even
...
func formatSetupComplete(result tui.SetupResult) string {
	lines := []string{"KajiCode setup complete"}
	...
	lines = append(lines, "next: "+setupCheckCommand(result.Provider.Name), "next: kajicode")
...
func setupTryThisExample(profile config.ProviderProfile) string {
	example := `kajicode exec "say hello in one short sentence"`
...
func setupCheckCommand(name string) string {
	...
	if name == "" {
		return "kajicode providers check --connectivity"
	}
	return "kajicode providers check " + setupCommandArg(name) + " --connectivity"
}
...
func writeSetupHelp(w io.Writer) error {
  ...
  kajicode setup [provider] [flags]

Guides first-run KajiCode setup. Without a provider, opens the setup UI. With a
provider catalog id, writes that provider as the active provider.

Examples:
  kajicode setup
  kajicode setup openai --api-key-env OPENAI_API_KEY
  kajicode setup ollama
  kajicode setup ollama-cloud
