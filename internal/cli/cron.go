...
// runCron is the dispatch entry for `kajicode cron`.
...
func runCronWith(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: kajicode cron <add|list|rm|pause|resume|run> ...")
		return exitUsage
	}
...
func cronAdd(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if expr == "" {
		fmt.Fprintln(stderr, "A cron expression is required (e.g. `kajicode cron add \"0 9 * * *\" --prompt ...`).")
		return exitUsage
	}
...
func writeCronHelp(w io.Writer) {
	fmt.Fprint(w, `kajicode cron — schedule agent jobs (foreground, file-backed)

Usage:
  kajicode cron add <cron-expr> [--prompt P | --recipe R] [--cwd D] [--model M] [--run-now]
  kajicode cron list
  kajicode cron pause <id> | resume <id> | rm <id>
  kajicode cron run [--once] [--catch-up] [id...]
