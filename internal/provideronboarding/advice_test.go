...
func TestSetupCommandUsesCatalogEnvAndSetActive(t *testing.T) {
	...
	want := "kajicode providers add groq --name fast --api-key-env GROQ_API_KEY --set-active"
	if got != want {
...
func TestSetupCommandForOpenAICustomAndLocalProviders(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			profile:   "custom",
			want:      "kajicode providers add custom-openai-compatible --name custom --api-key-env OPENAI_API_KEY",
		},
		{
			name:       "local provider",
			catalogID:  "ollama",
			profile:    "local",
			want:       "kajicode providers add ollama --name local",
...
func TestUseAndCheckCommands(t *testing.T) {
	if got, want := UseCommand("fast"), "kajicode providers use fast"; got != want {
		t.Fatalf("UseCommand() = %q, want %q", got, want)
	}
	if got, want := CheckCommand("fast", false), "kajicode providers check fast"; got != want {
		t.Fatalf("CheckCommand(false) = %q, want %q", got, want)
	}
	if got, want := CheckCommand("fast", true), "kajicode providers check fast --connectivity"; got != want {
...
func TestProviderActionsIncludesExpectedActions(t *testing.T) {
	...
	for _, action := range activeActions {
		if action.Command == "kajicode providers use fast" {
