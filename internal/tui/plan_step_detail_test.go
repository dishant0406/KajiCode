...
func TestPlanStepDetailAIExplanation(t *testing.T) {
	provider := &fakeProvider{events: []kajicoderuntime.StreamEvent{
		{Type: kajicoderuntime.StreamEventText, Content: "We added the new button and wired up its click."},
...
func TestPlanStepDetailSecondClickHides(t *testing.T) {
	provider := &fakeProvider{events: []kajicoderuntime.StreamEvent{
		{Type: kajicoderuntime.StreamEventText, Content: "late explanation"},
		{Type: kajicoderuntime.StreamEventDone},
	}}
	t0 := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	m := model{now: func() time.Time { return t0 }, provider: provider}
