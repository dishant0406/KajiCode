...
func mcpDoctorChecks(cfg config.MCPConfig) []BackendDoctorCheck {
	if len(cfg.Servers) == 0 {
		return []BackendDoctorCheck{{
			...
			Action:  "kajicode mcp add <name> --url <url>",
		}}
	}
	...
	for _, name := range names {
		...
		if raw.Disabled {
			checks = append(checks, BackendDoctorCheck{
				...
				Action:  "kajicode mcp enable " + target,
			})
			continue
		}
		...
		if err != nil {
			checks = append(checks, BackendDoctorCheck{
				...
				Action:  "kajicode mcp add " + target,
			})
			continue
		}
		detail := ""
		...
		check := BackendDoctorCheck{
			...
			Action:  "kajicode mcp check " + target,
		}
		if detail != "" {
