...
// `kajicode mcp` command, and PR/CI automation. The snapshot strips
...
// is exposed to the TUI render path, the headless `kajicode hooks`
...
// is exposed to the TUI render path, the headless `kajicode plugins`
...
// lane: MCP servers, hooks, and plugins. `kajicode doctor` and the
// headless `kajicode config` command can return one of these to give
