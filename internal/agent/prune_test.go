...
func bigToolResult(id string, tokens int) kajicoderuntime.Message {
...
func toolCallMsg(id, name string) kajicoderuntime.Message {
	return kajicoderuntime.Message{
		Role:      kajicoderuntime.MessageRoleAssistant,
		ToolCalls: []kajicoderuntime.ToolCall{{ID: id, Name: name}},
	}
}
...
func TestPrunePreservesRecentAndPrunesOld(t *testing.T) {
	...
	msgs := []kajicoderuntime.Message{
		...
		kajicoderuntime.Message{Role: kajicoderuntime.MessageRoleAssistant, Content: "thinking"},
		kajicoderuntime.Message{Role: kajicoderuntime.MessageRoleUser, Content: "next"},
	}
	...
	if out[1].ToolCallID != "old" || out[1].Role != kajicoderuntime.MessageRoleTool {
...
func TestPruneIsIdempotent(t *testing.T) {
	msgs := []kajicoderuntime.Message{
		...
		kajicoderuntime.Message{Role: kajicoderuntime.MessageRoleUser, Content: "go"},
	}
...
func TestPruneNeverTouchesNonToolMessages(t *testing.T) {
	msgs := []kajicoderuntime.Message{
		{Role: kajicoderuntime.MessageRoleUser, Content: strings.Repeat("u ", 60000)},
		{Role: kajicoderuntime.MessageRoleAssistant, Content: strings.Repeat("a ", 60000)},
	}
