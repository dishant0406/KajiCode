...
// taskOptions configures the `kajicode-perf-bench tasks` subcommand: the reproducible
// Terminal-Bench-style harness that runs KAJICODE headlessly against a task set and
...
func runTasksCommand(args []string, getenv func(string) string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[kajicode] Task benchmark failed: "+err.Error())
		return 1
	}
	...
	if !options.DryRun {
		...
		if binary == "" {
			_, _ = fmt.Fprintln(stderr, "[kajicode] Task benchmark failed: --binary is required unless --dry-run is set")
			return 2
		}
...
func taskHelpText() string {
	return strings.Join([]string{
		"Usage: kajicode-perf-bench tasks [options]",
		"",
		"Runs KAJICODE headlessly against a Terminal-Bench-style task set and records a",
		...
		"  --binary <path>     Path to the `kajicode` binary (required unless --dry-run)",
