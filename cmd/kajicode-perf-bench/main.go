...
func run(args []string, getenv func(string) string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[kajicode] Performance benchmark failed: "+err.Error())
		return 1
	}
	if options.Output != "" {
...
func parseArgs(args []string, getenv func(string) string) (cliOptions, error) {
	iterations, err := readPositiveIntegerEnv(getenv, "KAJICODE_PERF_ITERATIONS", defaultIterations)
	if err != nil {
		return cliOptions{}, err
	}
	warmupIterations, err := readNonNegativeIntegerEnv(getenv, "KAJICODE_PERF_WARMUP_ITERATIONS", defaultWarmupIterations)
	if err != nil {
		return cliOptions{}, err
	}
	coldStartP95Ms, err := readPositiveNumberEnv(getenv, "KAJICODE_PERF_COLD_START_WARN_MS", perfbench.DefaultThresholds.ColdStartP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	firstOutputP95Ms, err := readPositiveNumberEnv(getenv, "KAJICODE_PERF_FIRST_OUTPUT_WARN_MS", perfbench.DefaultThresholds.FirstOutputP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	harnessEndRssMaxMb, err := readPositiveNumberEnv(getenv, "KAJICODE_PERF_HARNESS_END_RSS_WARN_MB", perfbench.DefaultThresholds.HarnessEndRssMaxMb)
	if err != nil {
		return cliOptions{}, err
	}
	options := cliOptions{
...
func helpText() string {
	return strings.Join([]string{
		"Usage: kajicode-perf-bench [options]",
		"       kajicode-perf-bench tasks [options]   (Terminal-Bench-style task harness; see `tasks --help`)",
		"       kajicode-perf-bench turn  [options]   (per-turn tracing benchmark; see `turn --help`)",
		"",
		"Options:",
		...
		"  KAJICODE_PERF_ITERATIONS, KAJICODE_PERF_WARMUP_ITERATIONS",
		"  KAJICODE_PERF_COLD_START_WARN_MS, KAJICODE_PERF_FIRST_OUTPUT_WARN_MS",
