...
export GOCACHE="${GOCACHE:-/tmp/kajicode-go-cache}"
...
for goos in linux darwin windows; do
  ...
  compile_pkg "$goos" ./cmd/kajicode
done
compile_pkg linux ./cmd/kajicode-linux-sandbox
compile_pkg linux ./cmd/kajicode-seccomp
build_cmd windows ./cmd/kajicode-windows-command-runner kajicode-windows-command-runner.exe
build_cmd windows ./cmd/kajicode-windows-sandbox-setup kajicode-windows-sandbox-setup.exe

case "$(go env GOOS)" in
  linux)
    KAJICODE_SANDBOX_REAL_SMOKE=1 go test ./internal/sandbox -run 'TestLinuxHelperRealSandboxSmoke|TestLinuxLandlockRealSandboxSmoke' -count=1
    ;;
  darwin)
    go test ./internal/sandbox -run TestSandboxExecProfileAllowsDevNullAndTemp -count=1
    ;;
  windows)
    KAJICODE_SANDBOX_REAL_SMOKE=1 \
      KAJICODE_WINDOWS_COMMAND_RUNNER_EXE="$tmpdir/windows-kajicode-windows-command-runner.exe" \
      KAJICODE_WINDOWS_SANDBOX_SETUP_EXE="$tmpdir/windows-kajicode-windows-sandbox-setup.exe" \
