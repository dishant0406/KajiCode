...
KajiCode 是一个用于本地终端的 AI 编码智能体。它可以检查仓库、编辑文件、运行命令、使用浏览器/终端辅助工具，并在你选择模型和权限级别的同时保持持久的本地会话。

```bash
kajicode
kajicode exec "修复 ./pkg 中失败的测试"
...
- **保持控制权。** 文件写入、Shell 命令、网络访问和工作区外写入都经过 KajiCode 的权限和沙箱策略。
- **在终端中工作。** TUI 具有模型/提供商选择器、图片输入、斜杠命令、实时计划/工具渲染、回滚滚动、主题以及恢复/分叉支持。
- **无 TUI 也能工作。** `kajicode exec` 可脚本化，支持文本/JSON/stream-JSON I/O、隔离的工作树、规范优先运行，以及用于 CI 的有意义的退出码。
- **保持上下文本地化。** 会话存储在磁盘上，可搜索、可恢复，且 KajiCode 不会作为遥测数据上传。
...
go build -o kajicode-seccomp ./cmd/kajicode-seccomp   # 可选的兼容性包装器
```

将 `kajicode` 和 `kajicode-linux-sandbox` 放在 `PATH` 上的同一目录中（`~/.local/bin` 是一个好的默认选择）。macOS 不需要额外的辅助二进制文件。Windows 源码构建可以使用主 `kajicode.exe` 作为沙箱辅助程序；发布包仍然附带独立的 Windows 辅助可执行文件。
...
对于本地模型，运行 Ollama 或 LM Studio，然后使用 `kajicode setup` 或 `kajicode providers detect`。
...
- 不安全/自主模式是显式选择加入。
- 在 KajiCode 控制的界面上，密钥会从工具输出和日志中被脱敏。

示例：

```bash
kajicode --add-dir ../docs-site
kajicode exec --add-dir ../shared "更新两个仓库"
```
...
KajiCode 包含本地文件/搜索/编辑/Shell 工具、用于公共 URL 的 `web_fetch`，以及用于额外工具的 MCP 支持。

对于本地开发服务器，使用 Shell 命令（如通过 `exec_command` 的 `curl`），这样正常的沙箱和权限策略就会生效。长时间运行的命令保持附加到后台终端会话，可以从 TUI 中列出或停止。

npm 包还包含本地浏览器/终端工具使用的浏览器和终端辅助包。源码构建可以在它们位于 `PATH` 上或在 KajiCode 的本地控制设置中配置时使用相同的辅助工具。

## 常用命令

```text
kajicode                  交互式 TUI
kajicode exec             一次性或脚本化智能体运行
kajicode setup            首次运行提供商设置
kajicode auth             支持提供商的 OAuth/登录辅助
kajicode models           模型注册表和能力
kajicode providers        提供商配置和检测
kajicode doctor           设置、密钥和连接检查
kajicode context          上下文预算报告
kajicode repo-map         确定性仓库映射
kajicode repo-info        本地仓库摘要
kajicode search | find    搜索本地会话历史
kajicode sessions         检查、恢复、分叉和回滚会话
kajicode spec             管理规范模式草稿
kajicode specialist       管理专业子智能体
kajicode skills           管理 Markdown 指令技能
kajicode plugins          管理插件
kajicode hooks            管理生命周期钩子
kajicode mcp              管理 MCP 服务器和工具
kajicode serve --mcp      通过 MCP stdio 暴露 KajiCode 工具
kajicode sandbox          检查沙箱策略和授权
kajicode worktrees        准备隔离的 Git 工作树
kajicode verify           检测和运行本地验证检查
kajicode changes          检查和提交本地 Git 变更
kajicode usage            Token 使用量和估算成本
kajicode cron             定时智能体任务
kajicode update           检查更新版本
```
...
|---|---|
| `NO_COLOR=<任意值>` | 禁用颜色输出 |
| `KAJICODE_THEME=<名称>` | 选择启动主题（`auto`、`dark`、`light`，或颜色主题如 `dracula`、`nord`、`gruvbox`、`tokyo-night`、`catppuccin`、`one-dark`、`solarized-dark`、`rose-pine`、`everforest`、`neon`、`solarized-light`、`dune`） |
| `--theme <名称>` | 从 CLI 选择 TUI 主题（相同名称） |
| `/theme` | 在 TUI 中打开主题选择器（实时预览；`/theme <名称>` 直接切换） |
| `KAJICODE_NO_FADE=1` | 禁用流式淡入动画 |
...
KajiCode 基于 [MIT 许可证](LICENSE) 发布。
