...
KajiCode is distributed as:

- an npm package, `@dishant0406/kajicode`
...
`kajicode` wrapper plus, as an optional dependency, a platform payload with the
...
```bash
node "$(npm root -g)/@dishant0406/kajicode/scripts/postinstall.mjs"
```
...
```bash
KAJICODE_VERSION=0.1.0 scripts/install.sh
...
```bash
KAJICODE_INSTALL_DIR="$HOME/bin" scripts/install.sh
...
- Install path: `~/.local/bin/kajicode`
...
- Repository: `dishant0406/KajiCode`
- Version: latest GitHub release
- Install path: `%LOCALAPPDATA%\kajicode\bin\kajicode.exe`
...
go build -o kajicode-seccomp ./cmd/kajicode-seccomp
```

Put `kajicode` and `kajicode-linux-sandbox` in the same directory on `PATH`, for example
`~/.local/bin`. `kajicode-seccomp` is kept as a compatibility wrapper; the sandbox
...
KajiCode can run natively on Android via [Termux](https://termux.dev/). Build with
...
# Build KajiCode for Android
git clone https://github.com/dishant0406/KajiCode.git
cd KajiCode
CGO_ENABLED=0 GOOS=android GOARCH=arm64 go build -ldflags="-s -w" -o kajicode ./cmd/kajicode

# Move into PATH
mv kajicode ~/.local/bin/
```
...
> kills the process whenever KajiCode looks up a binary on `PATH` (git, sh, etc.).
...
```bash
pkg install proot
proot -b "$PREFIX/etc/resolv.conf:/etc/resolv.conf" kajicode
```

Create a wrapper at `~/.local/bin/kajicode` to avoid typing proot every time:

```bash
#!/data/data/com.termux/files/usr/bin/bash
exec proot -b "$PREFIX/etc/resolv.conf:/etc/resolv.conf" ~/.local/bin/kajicode.bin "$@"
```

**Scroll.** On native Termux (not under PRoot), mouse scrolling works out of the
box. The TUI uses Bubble Tea's `AllMotion` mouse mode by default. If you run KajiCode
...
**Providers.** KajiCode works with any OpenAI-compatible provider on Termux. For
example, to use OpenCode Zen's free tier:

```bash
kajicode providers add opencode \
  --name opencode \
...
Windows source builds can use the main `kajicode.exe` as the command runner and setup
helper through KajiCode's built-in self-dispatch path. If you want a release-style
layout anyway, build the standalone helper executables next to `kajicode.exe`:

```powershell
go build -o kajicode.exe ./cmd/kajicode
go build -o kajicode-windows-command-runner.exe ./cmd/kajicode-windows-command-runner
go build -o kajicode-windows-sandbox-setup.exe ./cmd/kajicode-windows-sandbox-setup
```
...
- `kajicode-v<version>-linux-<arch>.tar.gz`
- `kajicode-v<version>-macos-<arch>.tar.gz`
- `kajicode-v<version>-windows-<arch>.zip`
