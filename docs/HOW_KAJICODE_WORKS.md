...
This document explains KajiCode's architecture at a high level: the layers, module
...
KajiCode is a Go CLI application with two main user-facing surfaces:

- **Interactive TUI** (`kajicode`): a Bubble Tea application that renders chat,
  tool cards, permission prompts, model/provider pickers, and session controls.
- **Headless exec** (`kajicode exec ...`): a scriptable command runner that uses the
...
```mermaid
flowchart TD
    User[User / automation] --> CLI[cmd/kajicode -> internal/cli]
    ...
    Agent --> Provider[Provider runtime<br/>internal/kajicoderuntime.Provider]
...
The easiest way to understand KajiCode is not as a set of independent packages, but
...
| Package | Role |
| --- | --- |
| `cmd/kajicode` | Tiny binary entrypoint; calls `internal/cli.Run`. |
...
| `internal/kajicoderuntime` | Provider-neutral message, tool-call, stream, usage, and image types. Providers implement `Provider.StreamCompletion`. |
...
### Interactive TUI (`kajicode`)

```mermaid
sequenceDiagram
    participant Main as cmd/kajicode
...
`kajicode exec` uses the same `agent.Run` loop as the TUI, but without interactive
...
it. The model proposes an action, KajiCode decides whether that action is allowed to
...
TUI. KajiCode controls the loop mechanics: it keeps appending messages, enforces
...
The `Maybe compact context` step in the agent-loop diagram is KajiCode's context-window
...
A common source of confusion is that KajiCode has two event loops running at once:
...
- **Stream-JSON** is another surface over the same callbacks: `kajicode exec`
...
Providers are hidden behind this interface in `internal/kajicoderuntime`:
...
remote service. KajiCode stores the local event history; providers only receive the
messages needed for the current completion request.

## Stream-JSON Protocol

`kajicode exec --output-format stream-json` emits newline-delimited JSON events for
...
KajiCode's extension surfaces are loaded before the agent loop starts:
...
The same request in `kajicode exec` follows the same provider/tool/sandbox path, but
streaming events are written to stdout/stderr instead of rendered in Bubble Tea.

## Mental Model

When you work on KajiCode, start by locating the change on the four-layer spine from
